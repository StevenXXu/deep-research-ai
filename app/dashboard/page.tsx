    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      {showUpgradeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-sm border border-zinc-200 max-w-md w-full p-6 relative">
                <button
                ...
                <div className="text-center">
                    ...
                    <h3 className="text-xl font-bold text-zinc-900 mb-2">Out of Credits</h3>
                    ...
                    <a
                        ...
                        className="block w-full bg-zinc-900 hover:bg-zinc-800 text-white font-semibold py-3 px-4 rounded-lg transition-colors"
                    >
                        Upgrade Now
                    </a>
                    <button
      ...
      {/* Page Header */}
      <div>
        <h1 className="text-3xl font-bold text-zinc-900 tracking-tight">New Research Task</h1>
        <p className="mt-2 text-zinc-500">Enter a company URL or upload a pitch deck to generate a comprehensive pre-screen investment memo.</p>
      </div>
      ...
      <div className="bg-white p-8 rounded-xl border border-zinc-200 shadow-sm">
          ...
          {/* URL Field */}
          <div>
            <label htmlFor="url" className="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">Target URL or Project Name</label>
            <div className="relative rounded-md">
              ...
              <input
                ...
                name="url"
                id="url"
                className="block w-full pl-10 pr-4 py-3 bg-zinc-50 border border-zinc-200 rounded-lg focus:ring-1 focus:ring-zinc-900 focus:border-zinc-900 sm:text-sm transition-colors placeholder:text-zinc-400"
          ...
          {/* Language Field */}
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">Report Language</label>
            <select
              value={language}
              onChange={(e) => setLanguage(e.target.value)}
              className="w-full p-3 bg-zinc-50 border border-zinc-200 rounded-lg focus:ring-1 focus:ring-zinc-900 focus:border-zinc-900 transition-colors text-zinc-700"
            >
          ...
          {/* File Dropzone */}
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">Additional Context (Optional)</label>
            <div
              className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed border-zinc-200 rounded-lg hover:border-zinc-400 hover:bg-zinc-50/50 transition-colors cursor-pointer relative"
            ...
            >
              <div className="space-y-1 text-center">
                ...
                <div className="flex text-sm text-zinc-600 justify-center">
                  <span className="relative cursor-pointer bg-white rounded-md font-medium text-zinc-700 hover:text-zinc-900 focus-within:outline-none">
                ...
                <p className="text-xs text-zinc-400">PDF, DOCX, TXT up to 10MB</p>
                {file && (
                    <p className="text-sm text-emerald-600 font-semibold mt-2 flex items-center justify-center gap-2">
          ...
          <div className="pt-4">
            <button
              ...
              className={`w-full flex justify-center py-3.5 px-4 border border-transparent rounded-lg text-sm font-semibold text-white bg-zinc-900 hover:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zinc-900 transition-all ${loading ? 'opacity-75 cursor-not-allowed' : ''}`}
            >
              {loading && !jobId ? (
                ...
                {loading && jobId && (
                    <div className="space-y-3">
                        <div className="flex justify-between text-sm font-medium text-zinc-700">
                            ...
                            <span className="text-zinc-900 font-mono">{Math.round(progress)}%</span>
                        </div>
                        ...
                        <p className="text-xs text-zinc-400 text-center pt-2">This may take 2-3 minutes. You can leave this page.</p>
                    </div>
                )}

                {status && !loading && (
                    <div className={`mt-4 p-4 rounded-lg text-sm font-medium flex items-center gap-3 ${status.includes("Error") ? "bg-red-50 text-red-700 border border-red-200" : "bg-emerald-50 text-emerald-700 border border-emerald-200"}`}>
